...
16. Differential import (including `--force`) no longer overwrites an existing place Item’s UniqueId from folder meta. Overwriting could inject a value already used by another instance, causing Studio to refuse the file with “DM contains duplicate Unique ids”. The UniqueId already present on the matched Item is now preserved and re-emitted. Full rebuild path is unchanged.
