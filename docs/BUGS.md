...
- **2026-07-30 github-iterate residual scan (rule 0):** Open issues still 4 (borderline sparse). Fresh residual pass over residual notes + extract.py / build.py / attributes.py (unknown type_id early-stop) / bridge.py / xml_props.py / Luau / plugin / tests + code search for TODO/FIXME/HACK confirmed no additional elevatable true defects. Not-bugs / Intentional design section honored. Open remains 1–4 (child order, Attributes unknown-type, root non-Item, Live Sync rename noise).
