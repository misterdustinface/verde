...
- **2026-07-30 github-pickaxe residual + user report:** User-reported case where adding a directory caused an unrelated `Name_N` directory to be removed (treated as rename/orphan). Root cause: `_cleanup_orphaned_uniquified` ran under scripts-only/selective keep_map and treated legitimate (or previously-exported) `*_N` stems whose base was claimed this run as stale uniquify leftovers, then `rmtree`d them. Gated the cleanup to full `--all` exports only (this PR). No other elevatable defects found in the residual pass; Open remains 1–4.
...
31. `_cleanup_orphaned_uniquified` no longer runs under scripts-only or selective (`--root` / `--tag`) exports. Previously a kept base-named sibling caused any on-disk `Name_N` (legitimate Studio duplicate naming, or a previously-exported instance omitted by keep_map) to be treated as a stale uniquify leftover and `rmtree`d. Gating the aggressive cleanup to full `--all` hierarchy exports eliminates the false-positive data loss while preserving orphan removal when the entire tree is rewritten. (this PR)
