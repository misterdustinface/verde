...
12. `Verde.dumpScripts` / `restoreScripts` now encode the Tags attribute as a JSON array via `HttpService:JSONEncode` / `JSONDecode` (with a legacy comma-joined fallback on restore for older ScriptDump_* archives). Tags containing commas or other special characters are no longer truncated or split. Offline Python path was already safe (null-byte BinaryString).
