...
16. Differential import (including `--force`) no longer introduces UniqueId collisions. (a) The UniqueId already present on a matched place Item is always preserved and re-emitted (never overwritten from folder meta). (b) When the place Item has no UniqueId, the meta’s UniqueId is emitted only if it is not already used by another instance in the place; otherwise it is dropped. This also covers residual cases where on-disk Name / Name_2 leftovers share the same UniqueId value in their metas. Path matching already prevents a bare Foo_2 from landing on a place that only has Foo. Full rebuild path is unchanged.
